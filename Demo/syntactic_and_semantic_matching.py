...
def compute_similarity_score_and_rank_paths(given_sql_queries, control_flow_paths):
    """
    ...
    :param control_flow_paths: Control flow paths with database access
            ...
            one example of control_flow_path:
                ...
                (sql)
                select owner from owner owner left join fetch owner.pets where owner.id =:id select  from types where id=? select visit_date, description, null from visits where id=?

                (sql_list/database access)
                [select owner from owner owner left join fetch owner.pets where owner.id =:id]
                [select  from types where id=?]
                [select visit_date, description, null from visits where id=?]
    :return: Ranked control flow paths according to the similarity score between given SQL query and the database access of control path
    ...
    for control_flow_path in control_flow_paths:
        docs.append(control_flow_path[2])
    ...
    for control_flow_path in control_flow_paths:
        control_flow_paths_sql_query_list.append(control_flow_path[3])
...
def get_operator_table_name(sql_str):
    operator = sql_str.strip().rstrip().split(' ')[0].strip()
